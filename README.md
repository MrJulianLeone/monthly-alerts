...
- **Family & parent invites** — any user can invite a family member by email (tokened link). Adults invite other adults to start their own coaching. A minor with no parent yet can invite an adult family member: when that adult accepts, they are saved as the minor's parent (gaining the parent dashboard). See `POST /api/invites/family` and `/family-invite/[token]`.
